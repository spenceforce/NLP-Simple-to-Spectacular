...
parser.add_argument("-test", action="store_true", help="Run on small set of movie IDs.")
args = parser.parse_args()


def get_id(movie_id):
    r = requests.get(
        "http://www.omdbapi.com", params={"i": movie_id, "apikey": args.api_key}
    )
...
while len(id_map) < len(movie_ids):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future in tqdm(
            concurrent.futures.as_completed(future_to_id), total=len(ids_to_search)
        ):
