...
def get_id(movie_id):
    r = requests.get("http://www.omdbapi.com", params={'i': movie_id, 'apikey': args.api_key})
...
while len(id_map) < len(movie_ids):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        future_to_id = {executor.submit(get_id, movie_id): movie_id for movie_id in ids_to_search}
        for future in tqdm(concurrent.futures.as_completed(future_to_id), total=len(ids_to_search)):
...
pd.DataFrame([{'movie_id': k, 'canonical_movie_id': v} for k, v in id_map.items()]).to_csv("movie_ids.csv", index=False)
pd.DataFrame(unknown_ids, columns=['movie_id']).to_csv("unknown_movie_ids.csv", index=False)
